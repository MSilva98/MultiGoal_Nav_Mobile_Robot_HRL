...
class agentController():
    ...
    def getPath(self, paths, startGoal, finishGoal):
        ...
        start1 = startGoal+"1"
        finish1 = finishGoal+"1"
        start2 = startGoal+"2"
        finish2 = finishGoal+"2"
    ...
    def invertedPath(self, path, startGoal, finishGoal):
        ...
        if finishGoal[-1] == "1":
            finishState2 = self.highLvl.getGoalState(finishGoal[:-1]+"2")
        else:
            finishState2 = self.highLvl.getGoalState(finishGoal[:-1]+"1")
        startState = self.highLvl.getGoalState(startGoal)
        if startGoal[-1] == "1":
            startState2 = self.highLvl.getGoalState(startGoal[:-1]+"2")
        else:
            startState2 = self.highLvl.getGoalState(startGoal[:-1]+"1")
        newPath = dict()
        # Turn back as it will do inverse path from where it is
        newPath[finishState] = "back"
        newPath[finishState2] = "front"
        newPath[startState2] = "stop"
        for s in path:
            if s[0] != finishState and s[0] != finishState2 and s[0] != startState2 and s[0] != startState:
