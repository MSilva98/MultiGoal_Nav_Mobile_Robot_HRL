...
class agentController():
        ...
        while self.robot.step(self.timestep) != -1:    
            ...
            if self.highLvl.reachedSpecificGoal(cur_pos, finishGoal):
                ...
                if not first:
                    ...
                    next_highAction = self.highLvl.chooseAction(next_highState)
                    self.highLvl.updateQTable(next_highState, next_highState, next_highAction, r_t-last_t, finishGoal)
                last_t = r_t
                ...
                if nr_episodes < self.highLvl.episodes/4:
                    self.translation_field.setSFVec3f(init_pos1)
                    self.rotation_field.setSFRotation(init_ori1)
                    startGoal = self.highLvl.getGoalName(self.highLvl.getState((init_pos1[0],init_pos1[2], init_ori1[3]), checkingGoal=True))
                    ...
                    startGoal = self.highLvl.getGoalName(self.highLvl.getState((init_pos2[0],init_pos2[2], init_ori2[3]), checkingGoal=True))
                    ...
                    startGoal = self.highLvl.getGoalName(self.highLvl.getState((init_pos3[0],init_pos3[2], init_ori3[3]), checkingGoal=True))
                else:
                    self.translation_field.setSFVec3f(init_pos4)
                    self.rotation_field.setSFRotation(init_ori4)
                    startGoal = self.highLvl.getGoalName(self.highLvl.getState((init_pos4[0],init_pos4[2], init_ori4[3]), checkingGoal=True))
                self.robot_node.resetPhysics()
            ...
            elif self.highLvl.reachedSecondaryGoal(cur_pos, startGoal, finishGoal):
                middleGoal = self.highLvl.getGoalName(self.highLvl.getState(cur_pos, checkingGoal=True))
                print("From", startGoal, "found", middleGoal, "while training to reach", finishGoal, "\nSaving that info...")
                
                k = str((startGoal, middleGoal))
                if k not in paths:
                    paths[k] = statesPath.copy()
                else:
                    if len(paths[k]) >= len(statesPath):
            ...
            if next_highState != None:
                ...
                if not first and last_highAction != '':
                    self.highLvl.updateQTable(last_highState, next_highState, last_highAction, r_t-last_t, finishGoal)
    ...
    def getHighLvlAction(self, cur_pos):
        if self.corridor:
            ...
            if state != '':
                stateName = self.highLvl.getDoorName(state)
                # if lastState != '':
                #     lastName = self.highLvl.getDoorName(lastState)
                #     if stateName[:-1] != lastName[:-1]:
                #         print("StateName:", stateName, "S:", state, "Epsilon:", self.highLvl.epsilon)
