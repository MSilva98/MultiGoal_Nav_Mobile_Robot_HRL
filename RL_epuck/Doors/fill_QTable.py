...
class fill_QTable():
    ...
    def run(self):
        ...
        z_vals_lr_fwd = [z/100 for z in range(15, 36)]  # LR Door entrance
        z_vals_lr = [z/100 for z in range(-11, 21)]     # Inside LR Door
        
        z_vals_fwd = [z/100 for z in range(-25, 33)] # Door entrance up to door exit
        z_vals_fwd += [z/100 for z in range(74, 97)] # Dead End
        
        rot_vals_r = [rot*math.pi/180 for rot in range(90, -180, -1)]   # Used in FR Door -> 90 to 180 degrees ignored in FR Door action to right (assumed positions won't happen)
        rot_vals_fwd = [rot*math.pi/180 for rot in range(180, -180, -1)]       # 0 to 360 degrees used in FR Door action to front (angle conversion makes this table suitable for FL Door aswell (270 to 90 is FR Door, 90 to 270 is FL Door))
        rot_vals_lr = [rot*math.pi/180 for rot in range(90, -180, -1)]   # Used in LR Door
        rot_vals_lr_fwd = [rot*math.pi/180 for rot in range(90, -90, -1)]   # Used in LR Door entrance

        # Load corridor QTable
        f = open("../Corridor/QTable_v5_17500h_corrected.txt", "r")
        # f = open("../Corridor/QTable_v5_13000h_corrected.txt", "r")
        # f = open("../Corridor/QTable_v5_16500h_corrected.txt", "r")
        # f = open("../Corridor/QTable_v5_15500h_corrected.txt", "r")
        # f = open("../Corridor/QTable_v6_walls_2.txt", "r")
        ...
        for door in doorTypes:
            ...
            for act in actions:
                f2 = open("./v52/QTABLE_LOG_"+door+"_"+act+".txt", "w+")
                ...
                if act == "Front":
                    x_vals = x_vals_fwd
                    if door == "LR":
                        z_vals = z_vals_lr_fwd
                        rot_vals = rot_vals_lr_fwd
                    else:
                        z_vals = z_vals_fwd
                    ...
                    for z in z_vals:
                        ...
                        for rot in rot_vals:
                            ...
                            if act == "Front":
                                ...
                                if door == "LR": # LR Door entrance
                                    p = (x_transformed, 0.1, cur_ori)
                                else:
                                    # Corridor dead-end
                                    if z > 0.73:
                                        ...
                                        if cur_ori > 90:
                                            ...
                                            if x_transformed != 0.0:
                                                x_transformed = -x_transformed
                                        p = (x_transformed, 0.15, cur_ori)  # 0.15 is a position considered to be middle of corridor (situation where rotation is symmetric: -80º=100º)
                            else:
                ...
                self.brain.saveQTable("./v52/QTable_v5_17500h_"+door+"_"+act+".txt")
                # self.brain.saveQTable("./lr/QTable_v5_16500h_"+door+"_"+act+".txt")
                # self.brain.saveQTable("./lr/QTable_v5_15500h_"+door+"_"+act+".txt")
                # self.brain.saveQTable("./lr/QTable_v5_13000h_"+door+"_"+act+".txt")
                # self.brain.saveQTable("./lr/QTable_v6_"+door+"_"+act+".txt")
                print("TABLE SAVED!")
