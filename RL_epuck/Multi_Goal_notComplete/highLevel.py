...
class Agent():
    ...
    def getState(self, state, checkingGoal=False):
        ...
        if checkingGoal:
            ...
            for s in all_states:
                if s in self.QTable.keys() and s not in self.goals.keys():
                    return s
    ...
    def reachedGoal(self, state, goal):
        tableState = self.getState(state, checkingGoal=True)
        return tableState in self.goals and self.goals[tableState] == goal

    def reachedSecondaryGoal(self, state, startGoal, finishGoal):
        tableState = self.getState(state, checkingGoal=True)
        return tableState in self.goals and self.goals[tableState] != startGoal and self.goals[tableState] != finishGoal
