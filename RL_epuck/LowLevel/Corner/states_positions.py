...
# python script to get all possible states in corner for each X,Z,theta
...
class states_positions():
    ...
    def run(self):
        ...
        z_vals = [z/100 for z in range(-96, -73)]  # -96 to -74 is Corner
        # 360 degrees in radians
        rot_vals = [x*math.pi/180 for x in range(0,360,10)]
        ...
        for rot in rot_vals:                    
            ...
            for x in x_vals:
                for z in z_vals:
                    ...
                    cur_x = round(x+0.85,2)
                    cur_z = round(z+0.85,2)
                    print(cur_x, cur_z, rot)

                    state = self.brain.sensorsToState(dsValues, True)
            ...
            with open("state_maps/4_states_"+str(round(rot*180/math.pi,0))+".txt", "w+") as jsonFile:
